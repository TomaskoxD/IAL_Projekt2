...
void htPrintItem(ht_item *ptritem)
{
	if (ptritem)
	{
...
void htPrintTable(ht_item_t *ptrht)
{
	int maxlen = 0;
	int sumcnt = 0;
	int sumcnt2 = 0;
...
void use_search(ht_item_t *ptrht, tKey key)
{
	solved = 1;
	ht_item *ptritem = htSearch(ptrht, key);
	if (!solved)
	{
...
void use_insert(ht_item_t *ptrht, tKey key, tData data)
{
	solved = 1;
...
/* vola funkci htRead a tiskne vysledek,
 v pripade ze neni resena tiskne chybu */
void use_read(ht_item_t *ptrht, tKey key)
{
	solved = 1;
...
void use_delete(ht_item_t *ptrht, tKey key)
{
	solved = 1;
	htDelete(ptrht, key);
	if (!solved)
...
void use_clear_all(ht_item_t *ptrht)
{
	solved = 1;
	htClearAll(ptrht);
	if (!solved)
...
{
	UNDEFPTR = (ht_item *)malloc(sizeof(ht_item_t));
	UNDEFPTR->key = "*UNDEF*";
	UNDEFPTR->data = -1;
	UNDEFPTR->ptrnext = NULL;
	ptrht = (ht_item_t *)malloc(sizeof(ht_item_t));
