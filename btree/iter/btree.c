...
void bst_replace_by_rightmost(bst_node_t *target, bst_node_t **tree) // POGU ide, ale je to clownfiesta, nejde free
{
  ...
  {
    temp_left = *tree;
    // printf("\ntlacim nodu temp, target : ");
    // bst_print_node(temp_left);
    // bst_print_node(target);
    target->key = temp_left->key; // aktualizujem hodnoty
    target->value = temp_left->value;
    free(temp_left);
    target->left = NULL;
    temp_left = NULL;
    // printf("\n");
...
void bst_delete(bst_node_t **tree, char key) // ani nahodou
{
  ...
  // printf("tlacim nodu tree, current : ");
  // bst_print_node(temp);
  // bst_print_node(parent);
  ...
  else if (current->right == NULL) // ak ma len laveho potomka
  {
    ...
    else
    {
      ...
      bst_node_t *pointer_left = NULL;
      bst_node_t *pointer_right = NULL;

      //bst_print_node(pointer_left);

      if (current->left->right != NULL)
      {
        pointer_right = current->left->right;
      }
      if (current->left->left != NULL)
      {
        pointer_left = current->left->left;
      }
      free(current->left);
      current->right = pointer_right;
      current->left = pointer_left;
      //current->left = NULL;
      //parent->left = temp;
    }
  }
  else if (current->left == NULL) // ak ma len praveho potomka
  {
    ...
    else
    {
      ...
      {
        pointer_right = current->right->right;
      }
      if (current->right->left != NULL)
      {
        pointer_left = current->right->left;
      }
      free(current->right);
      current->right = pointer_right;
      current->left = pointer_left;
